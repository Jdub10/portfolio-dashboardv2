...
class Components:
    ...
    def render_health_strip(stats: dict, fx_rate: float, last_update: str):
        ...
        <div class="health-strip">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                ...
                <span style="font-size:0.72rem;color:#64748b;">Updated {last_update}</span>
            </div>
            <div class="health-grid">
                <div class="health-item">
                    <div class="health-label">Net Liquidation</div>
                    <div class="health-value">${stats['total_mv']:,.0f}</div>
                    <div class="health-delta delta-neutral">AUD</div>
                </div>
                <div class="health-item">
                    <div class="health-label">Lifetime P&L</div>
                    <div class="health-value {pnl_cls}">{pnl_arrow} ${stats['total_pnl']:,.0f}</div>
                    <div class="health-delta {pnl_cls}">{stats['pnl_pct']:+.2f}%</div>
                </div>
                <div class="health-item">
                    <div class="health-label">Cash</div>
                    <div class="health-value">${stats['cash_value']:,.0f}</div>
                    <div class="health-delta delta-neutral">{cash_pct:.1f}% · {cash_status}</div>
                </div>
                <div class="health-item">
                    <div class="health-label">Concentration</div>
                    <div class="health-value">{hhi:.2f}</div>
                    <div class="health-delta delta-neutral">HHI · {hhi_label}</div>
                </div>
                <div class="health-item">
                    <div class="health-label">AUD/USD</div>
                    <div class="health-value">{fx_rate:.4f}</div>
                    <div class="health-delta delta-neutral">{stats['num_positions']} positions</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    def render_bucket_cards(bucket_df: pd.DataFrame):
        ...
        for _, row in bucket_df.iterrows():
            ...
            cards_html += f"""
            <div class="bucket-card" style="border-color:{color};">
                <div class="bucket-header" style="color:{color};">{label}</div>
                <div class="bucket-current" style="color:{color};">{current:.1f}%</div>
                <div class="bucket-target">Target: {target:.0f}%</div>
                <div class="bucket-gap">{gap_display}</div>
                <div class="bucket-progress">
                    <div class="bucket-progress-fill" style="width:{progress:.0f}%;background:{color};"></div>
                </div>
                <div class="bucket-footer">
                    {positions} position{'s' if positions != 1 else ''} · ${mv:,.0f}
                </div>
            </div>
            """
        cards_html += '</div>'
    ...
    def render_position_list(pos_df: pd.DataFrame, selected_bucket: str = "All"):
        ...
        for _, row in filtered.iterrows():
            ...
            <div class="position-card" style="border-left-color:{color};">
                <div class="pos-row-1">
                    <div>
                        <span class="pos-ticker">{row['Ticker']}</span>
                        <span style="font-size:0.75rem;color:#64748b;margin-left:6px;">{bucket}{sector_str}</span>
                    </div>
                    <span class="pos-action {action_class}">{action_icon} {action}</span>
                </div>
                <div class="pos-row-2">
                    <span class="pos-value">${row['MV_AUD']:,.0f}</span>
                    <span class="pos-pnl {pnl_cls}">{pnl_arrow} {row['PnL_Pct']:+.1f}% (${row['PnL_AUD']:,.0f})</span>
                </div>
                <div class="pos-detail">
                    Weight: {row['Weight_%']:.1f}% / {row['Target_%']:.1f}% · {gap_info}
                </div>
            </div>
            """
    ...
    def render_cleanup_alert(bucket_df: pd.DataFrame, pos_df: pd.DataFrame):
        ...
        <div style="background:#fee2e2;border:2px solid #dc2626;border-radius:12px;padding:14px 16px;margin:12px 0;">
            <div style="font-size:0.9rem;font-weight:700;color:#991b1b;margin-bottom:6px;">
                🔴 Cleanup Queue: {cleanup_pct:.1f}% of portfolio needs exit
            </div>
            <div style="font-size:0.82rem;color:#7f1d1d;">
                ${cleanup_mv:,.0f} AUD in non-framework positions. Target: 0%. 
                {len(cleanup_positions)} position{'s' if len(cleanup_positions) != 1 else ''} to exit at break-even.
            </div>
        </div>
        """, unsafe_allow_html=True)
